...
4. Sau khi profile đã được public:
   ...
   * Admin có thể kiểm tra trạng thái thanh toán của từng section ở danh sách yêu cầu cập nhật profile.
...
flowchart TD
    ...
    UserClick --> CalcFee[Tính tổng chi phí đăng tin\n1 lần duy nhất bao gồm tất cả các chi phí khác từ mỗi section post, image, posting service]
    ...
    DoPublic --> ChangeBtn[Ẩn nút đăng hồ sơ, hiển thị nút Gia hạn tin]
    ChangeBtn --> EndPublic([Kết thúc đăng hồ sơ])
    
    %% ===== LUỒNG CẬP NHẬT SAU PUBLIC =====
    EndPublic --> UpdateStart[User cập nhật hồ sơ\nthêm service / ảnh]
    
    UpdateStart --> CalcExtra[Tính chi phí phát sinh khi click Lưu\ntheo từng section]
